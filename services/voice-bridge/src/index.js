...
import { fetchAgent, runTurn, synthTts } from "./lovable";
...
function normalizeVoicemailText(value) {
    return value
        .toLowerCase()
        .replace(/[^a-z0-9' ]+/g, " ")
        .replace(/\s+/g, " ")
...
function isVoicemailSystemText(value) {
    const text = normalizeVoicemailText(value);
    if (!text)
        return false;
    return (/\b(?:to|you(?:'re| are)? (?:being )?(?:forwarded|sent|redirected))\s+(?:voice\s*mail|voicemail)\b/.test(text) ||
    ...
    websocket: {
        ...
        async message(ws, raw) {
            ...
            if (msg.event === "start") {
                ...
                if (!speakFirst)
                    return; // agent stays silent until user speaks
...
function isNoise(text, confidence) {
    ...
    if (confidence > 0 && confidence < 0.5 && words.length < 4)
        return true;
    if (words.length === 1) {
        // A lone greeting/acknowledgement is real input; a lone grunt is not.
        if (NOISE_ONLY.has(words[0]) && !["hello", "hi", "yes", "no"].includes(words[0]))
            return true;
        if (words[0].length <= 2)
            return true;
    }
...
function openDg(session) {
    session.dg = openDeepgram(DEEPGRAM_KEY, {
        ...
        onFinal: (t, confidence) => {
            ...
            if (session.speaking && text.split(/\s+/).length < 3)
                return;
        ...
        onError: (e) => log("error", "deepgram_error", {
            connection_id: session.connectionId,
            call_sid: session.callSid,
            error: String(e),
        }),
...
async function handleUserTurn(session, userText) {
    ...
    try {
        const { reply, end_call, end_reason } = await runTurn(session.agent, session.history, session.callSid);
        if (!reply)
            return;
...
async function speak(session, text) {
    if (!session.agent || !session.streamSid || session.closed)
        return;
    let cancelled = false;
    session.cancelSpeech = () => {
        cancelled = true;
    ...
    try {
        const { audio_url } = await synthTts(text, session.agent.voice_id, session.agent.language, session.agent.tts_engine, session.agent.voice_settings);
